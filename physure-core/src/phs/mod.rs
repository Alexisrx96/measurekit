...
pub fn eval_phs(input: &str) -> crate::error::PhysureResult<Vec<PhsValue>> {
    ...
    while let Some(start_idx) = input[pos..].find("```") {
        ...
        if let Some(end_idx) = input[text_start..].find("```") {
            ...
            results.push(PhsValue::String(clean_text.to_string()));
            pos = abs_end + 3;
        } else {
            let text_content = &input[text_start..];
            results.push(PhsValue::String(text_content.to_string()));
            pos = input.len();
            break;
        }
    }
