...
class TestDimension(BaseTestUnit):
    ...
    def setUp(self):
        super().setUp()
        register_dimension(Dimension({"L": 1}), "Length")
        register_dimension(Dimension({"M": 1}), "Mass")
        register_dimension(Dimension({"T": 1}), "Time")
    ...
    def test_init_and_new(self):
        ...
        # Test caching through __new__ (same exponents should return
        # same instance)
        dim2 = Dimension({"L": 1})
        ...
        self.assertIs(
            dim4, dim1
        )  # Same as first dimension after normalization
    ...
    def test_string_representation(self):
        ...
        self.assertEqual(repr(dim4), "<Dimension: L·T⁻¹ {'L': 1, 'T': -1}>")

        # Test empty dimension
        dim5 = Dimension({})
        self.assertEqual(str(dim5), "Dimensionless")
        self.assertEqual(repr(dim5), "<Dimension: Dimensionless {}>")
