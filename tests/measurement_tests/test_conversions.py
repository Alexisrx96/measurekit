# measurekit/tests/measurement_tests/test_conversions.py
...
class TestUnitRegistry(unittest.TestCase):
    ...
    def test_get_conversion_factor(self):
        ...
        self.assertEqual(
            get_conversion_factor(self.length, "km", "cm"), 100000.0
        )
        with self.assertRaises(ValueError):
            get_conversion_factor(self.length, "unknown", "m")

        with self.assertRaises(ValueError):
            get_conversion_factor(self.time, "m", "s")
...
class TestCompoundUnitConversion(unittest.TestCase):
    ...
    def test_compound_factor(self):
        ...
        self.assertAlmostEqual(compound_factor(force_cgs), 1e-5)
        with self.assertRaises(ValueError):
    ...
    def test_get_compound_unit_conversion_factor(self):
        ...
        self.assertEqual(
            get_compound_unit_conversion_factor(meter, centimeter), 100.0
        )
        ...
        self.assertAlmostEqual(
            get_compound_unit_conversion_factor(velocity_mps, velocity_kmph),
            3.6,
        )
        ...
        # <-- CORREGIDO: Usar el nombre de función correcto
        self.assertAlmostEqual(
            get_compound_unit_conversion_factor(force_newton, force_dyne), 1e5
        )

        length = CompoundUnit({"m": 1})
        time = CompoundUnit({"s": 1})
        with self.assertRaises(ValueError):
            get_compound_unit_conversion_factor(length, time)
